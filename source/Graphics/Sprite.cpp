...
// - 深度テスト・ライティングを無効化して描画し、終了後に復元する
// NOTE: SetWorldViewProjection2D() はワールド行列を Identity にリセットする
// NOTE: 2D スプライトは法線を持たないため、ライトを無効化しないと暗く描画される
void Sprite::Draw()
{
    if (!m_VertexBuffer) return;
    ...
    // 2D スプライトはライティング不要のため無効化
    // NOTE: ライトを有効のままにすると法線 (0,0,0) により ndl=0 となり暗く描画される
    LIGHT disabledLight{};
    disabledLight.Enable = FALSE;
    Renderer::SetLight(disabledLight);
