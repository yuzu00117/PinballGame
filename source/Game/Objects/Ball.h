...
//
// NOTE:
// - GetVelocity() は m_RigidBody に依存するため、Init() 後でないと使用できない
//------------------------------------------------------------------------------
#pragma once

#include "GameObject.h"
#include "Vector3.h"
#include "RigidBody.h" // m_RigidBodyの定義が必要なため
...
public:
	...
	/// 速度を取得する（RigidBody 側の速度の参照）
	/// 戻り値：参照（呼び出し側で変更可能）
	/// NOTE: Init() 呼び出し前（RigidBody 生成前）に使用するとクラッシュするため注意
	Vector3& GetVelocity() { return m_RigidBody->m_Velocity; }
...
private:
	...
	// ボールのモデルパス
	//------------------------------------------------------------------------------
	inline static const std::string kBallModelPath = "asset//model//ball.obj";
