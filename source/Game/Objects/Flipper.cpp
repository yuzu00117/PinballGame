...
// 役割:
// Flipperクラスの実装。
// フリッパーの初期化、アーム生成、キー入力による回転、ボールとの衝突（弾く動作）を実装する。
//
// 設計意図:
// - 親オブジェクトは回転軸として機能し、回転制御のみを行う
// - アームは子オブジェクトとして生成し、形状（BoxCollider）と描画（MeshRenderer）を持つ
// - 衝突コールバック OnCollisionStay にて、ボールに力を加える簡易物理を実装している
//
// 構成:
// - Init: アーム生成、基準角度設定
// - Update: 回転制御
// - OnCollisionStay: 反発処理
//
// NOTE:
// - 衝突時の反発力計算は簡易的なものであり、物理エンジンの本来の反発係数とは異なる挙動をする
//------------------------------------------------------------------------------
#include "Flipper.h"
#include "Input.h"
